...
- 데이터 로딩: Google Sheets Visualization API (JSONP)
...
|---|---|---|
| `--bg` | `#111827` | 배경 |
| `--accent` | `#16988d` | 강조 (포인트 컬러) |
| `--text` | `#ffffff` | 본문 텍스트 |
| `--sub` | `#8E95A0` | 보조 텍스트·노트·비활성 버튼 |
| `--border` | `#1e293b` | 구분선 |
...
- **하이라이트 박스** — `border: 3px solid --accent` / `background: rgba(22,152,141,0.12)` / `border-radius: 14px` / 패턴의 핵심 부분 강조용
- **사이드바** — 너비 `min(440px, 92vw)`, 좌측 슬라이드 인입, `border-right: 2px solid --accent`, `box-shadow: 5px 0 30px rgba(0,0,0,0.6)`
- **유닛 버튼** — pill 형태 (`border-radius: 20px`), 기본: 투명 배경 + `--sub` 텍스트, 활성: `--accent` 배경 + 검정 텍스트
- **인덱스 항목** — `border-left: 4px solid transparent`, 활성 시 accent 색 border + `rgba(22,152,141,0.18)` 배경
- **인덱스 태그** — `font-size: 0.72em`, `background: rgba(22,152,141,0.12)`, `border-radius: 4px`, 대문자
- **단축키 바** — 하단 중앙 고정, 투명 배경, 기본 `opacity: 0.6`, 활성 시 `opacity: 1 + --accent`
- **단축키 뱃지 (`.kb`)** — `background: #2a2a2a`, `border: 1px solid #444`, `border-radius: 3px`
- **시작 오버레이** — 로고 + 텍스트 + pill 버튼, 배경 `rgba(10,15,24,0.92)`, 버튼 hover 시 `--accent` 채움

#### 내비게이터 Navigator Logic

슬라이드는 시트에 입력된 순서 그대로 노출된다. 별도 재정렬 없이 시트 순서가 곧 화면 순서다.

왼쪽 사이드바 인덱스에는 `ptnA` 타입만 표시된다. 항목을 누르면 해당 슬라이드로 바로 이동한다.

화면 상단의 섹션 탭은 현재 유닛 내에서 해당 타입이 처음 등장하는 슬라이드로 점프하는 단축키다. 해당 타입이 없으면 탭은 비활성 처리된다.

| 탭 | 점프 대상 |
|---|---|
| 패턴 Pattern | 유닛 내 첫 번째 `ptnA` 슬라이드 |
| 드릴 Drill | 유닛 내 첫 번째 `drillA` 슬라이드 |
| 표현 Expression | 유닛 내 첫 번째 `expression` 슬라이드 |
| 칼리마트 Kalimat | 유닛 내 첫 번째 `kalimat` 슬라이드 |
| 나스 Nass | 유닛 내 첫 번째 `nass` 슬라이드 |
...
```
https://storage.googleapis.com/all-that-arabic-14/listening/{key}.mp3
```

실패 시 `.wav`로 폴백. key가 곧 파일명이므로 파일 목록 탐색 없이 바로 요청하여 로딩 지연이 없다. 모든 오디오 파일은 타입 구분 없이 `listening/` 폴더 하나에 보관한다.

> **1.4.3 → 1.4.4 변경점**: URL 구성 기준이 `id_datasheet` → `key`로 바뀜. 앱 코드의 `getAudioUrls()` 함수에서 id 대신 key를 참조하도록 수정 필요.
