...
오디오 파일은 ID와 동일한 이름으로 GCS에 저장한다 (유닛 prefix 없음).

```
listening144/{id}.mp3   예: listening144/ptn_ba-bu-bi.mp3
```

유닛 prefix를 두지 않는 이유:
- 유닛 재배치 시 파일을 옮길 필요 없음 (ID는 type+note 기반이라 globally unique)
- 매칭 로직 단순 (직접 URL 조합 → 단일 GET)

ID가 `type_note` 기반이므로, 같은 표현이 여러 유닛에서 등장해도 같은 오디오 파일을 공유한다. 다른 유닛에서 다른 발음으로 녹음하고 싶으면 note를 다르게 입력해 ID를 분리한다.

**아랍어 탭 → 오디오 매칭** — 모바일 교재 앱에서 아랍어 텍스트를 누르면 해당 행의 ID로 URL을 조합해 재생한다. 시트의 `url` 컬럼이 채워져 있으면 그 값을 우선 사용한다.
...
올댓아라빅 인쇄 교재의 페이지 양식. **유닛 1개 = 4종(표현·패턴·드릴·나쓰)의 페이지로 구성된 한 묶음**이며, 모든 페이지는 B5 사이즈로 디자인되어 화면 미리보기 = 인쇄물 1:1 매칭이다. 구현 파일: `ata144_original/ata144_original.html`.
...
- **오디오 재생 인터랙션**: 모든 카드/셀에서 아랍어를 가리키면 재생되는 방식
  - 데스크톱: 마우스 호버 시 자동 재생
  - 모바일: 터치 시 재생
  - 시각적 재생 버튼 UI는 두지 않음 (텍스트 자체가 인터랙션 대상)
  - 오디오 URL: 시트의 `url` 컬럼 우선, 없으면 `https://storage.googleapis.com/all-that-arabic-14/listening(144)/{id}.mp3` (id = `id_drill` 또는 `id_kalimat`)
...
- **학생용 교재 앱**: 인쇄용 B5 교재와 모바일 교재 앱은 같은 반응형 값을 공유하지 않는다. B5 출력용 값은 `ata144_original/ata144_original.html` 기준으로 유지하고, 학생용 교재 앱(`ata144_textbook/ata144_textbook.html`)은 별도 디자인 코드로 정의한다.
...
**구현 위치**: `ata144_original/ata144_original.html` 상단 상수 + 헬퍼 함수
