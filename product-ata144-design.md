...
**단어(kalimat)**: `kalimat` 탭의 `type=kalimat` 행. 표현(exp)과 동일한 좌(한국어)-우(아랍어+음가) 페어 카드 1열. 라하 깃발·탭 오디오 동일 지원. **`kalimat+`** 행은 일반 `kalimat` 다음 **최하단**에 모아 표시하며, 아주 옅은 회색 음영(`.m-card.kalimat-plus { background:#f5f6f9 }`)으로 미세하게 구분(주로 구어 변형 어휘). 빈 arabic 행 제외.
...
표현/패턴/드릴/지문/**단어** 5개 섹션. 한 유닛의 페이지를 글로벌 시퀀스로 평탄화. 페이지 단위는 인쇄 교재와 1:1 매핑(권장) 또는 느슨한 매핑(추후 결정).

**드릴 `page` 열 (2026-05-18~)**: drill 행에 `page` 값이 있으면 같은 유닛 내 같은 `page` = **한 화면(스와이프 1장)**. 값 없는 유닛(1·2과 등)은 기존 동작(드릴 전체 1페이지) 유지. 빈 행(arabic 없음)은 수집 단계에서 제외(유령 페이지 방지). 선생님앱도 동일하게 `page` 오름차순으로 드릴 슬라이드 정렬(값 없으면 기존 ref_ptn 로직). 일반 원칙은 3과 검증 후 확정 예정 — `product-ata144.md` 이슈란 "드릴 `page` 열" 참조.
...
- **`lineDir` = 첫 강한문자만** (최종 규칙, 음가/한글 등 예외 없음):
  - 아랍어로 시작 → **RTL**
  - 다른 언어로 시작 → **LTR**
  - 불릿 row·안쪽 줄 모두 동일 기준. text-align은 불릿 방향(첫 줄)에 맞춤.
  - (한글·로마자 유무 기반 휴리스틱은 케이스마다 충돌해 폐기. 단순 규칙으로 통일)
- 혼합 한 줄 불릿(`دَرَسَ., darasa. 그가...`)은 아랍어 시작이라 RTL — 아랍어가 오른쪽(점 옆), 나머지는 한 LTR 런으로 왼쪽. 깔끔히 분리하려면 다중 줄 불릿(아랍어 줄 / 번역 줄)으로 작성 권장.
