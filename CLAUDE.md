...
1. **격모음(إعراب) 미표기** — 명사의 마지막 모음은 표시하지 않는다.
   ...
   - **예외 (2026-07-27 확정): 만숩 탄윈(ـً)은 나오는 모든 경우 표기하고, 음가(note)에도 `-an`으로 발음을 적는다.**
     예: أَسْأَلُ سُؤَالًا (asʔala suʔālan) ✓ / سُؤَال ✗ — أَشْرَبُ قَهْوَةً (qahwatan) ✓ (شُكْرًا·غَدًا·كَثِيرًا와 동일 원리)
     단, 2변화 명사(ممنوع من الصرف — 색깔 أَسْوَد، 여성형 حَمْرَاء 등)는 만숩이어도 탄윈이 없으므로 무표기 유지
...
- 이름(alam 타입)은 사용 가능
- **여성 2인칭 금지 (2026-07-27 확정)**: 상대를 여성형으로 부르는 형태(ـِين 동사, ـتِ 완료형, أَنْتِ, 여성 명령형 تَفَضَّلِي 등)는 지문·드릴·래퍼·스크립트 전체에서 쓰지 않는다. 여성 화자는 1인칭·중립 발화로만 등장한다(발화 안에 여성 표지가 필요한 경우 1인칭 آسِفَة류만 허용). A02~A08 인사 유닛의 남녀 인사쌍(كِيفَك/كِيفِك 등)은 예외 여부 확정 전까지 기존 유지.
- **한국어 해석은 방언·사용역과 무관**: 표준어=존댓말, 구어=반말 대응을 하지 않는다. 같은 문형 그룹(패턴·래퍼·드릴)은 높임을 통일하고, 지문은 대화 맥락(관계·장면)에 따라 자연스럽게 정해 한 대화 안에서 일관 유지.
